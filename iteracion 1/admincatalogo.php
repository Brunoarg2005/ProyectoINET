...
<div class="productoContainer">
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            ...
            echo '</div>';
            echo '<div>';
            echo '<button onclick="window.location.href=\'editProduct.php?id=' . $row['id_producto'] . '\'">Editar</button>';
            echo '<button onclick="window.location.href=\'deleteProduct.php?id=' . $row['id_producto'] . '\'">Eliminar</button>';
            echo '</div>';
            echo '</div>';
        }
    } else {
...
function editProduct(productId) {
    window.location.href = 'editProduct.php?id_producto=' + productId;
}
